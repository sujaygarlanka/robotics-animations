...
class KDTreeNode:
    def __init__(self, dim=None, point=None, leaf=False):
        if not leaf:
            self.dim = dim
...
class KDTree(Scene):
    ...
    def _find_nearest_neighbor(self, kd_tree, point):
        ...
        # circle = DashedVMobject(circle, num_dashes=20)
            ...
            if node.leaf:
                ...
                for p in node.children:
                    ...
                    if min(distance, nn[0]) == distance:
                        ...
                        self.play(
                            # circle.animate.scale(scale_factor),
    ...
    def _create_kd_tree(self):
        ...
        self._write_info(
            ...
            """, {})
        # B leaf node left
        b_node_leaf_left = KDTreeNode(leaf=True)
        ...
        # B leaf node right
        b_node_leaf_right = KDTreeNode(leaf=True)
        square = Square(side_length=0.6, stroke_color=BLACK, fill_opacity=0.0, stroke_width=3)
        ...
        self._write_info(
            ...
            """, {})
        # D leaf node left
        d_node_leaf_left = KDTreeNode(leaf=True)
        square = Square(side_length=0.6, stroke_color=BLACK, fill_opacity=0.0, stroke_width=3)
        ...
        # D leaf node right
        d_node_leaf_right = KDTreeNode(leaf=True)
        square = Square(side_length=0.6, stroke_color=BLACK, fill_opacity=0.0, stroke_width=3)
        ...
        # E leaf node left
        e_node_leaf_left = KDTreeNode(leaf=True)
        square = Square(side_length=0.6, stroke_color=BLACK, fill_opacity=0.0, stroke_width=3)
        ...
        # E leaf node right
        e_node_leaf_right = KDTreeNode(leaf=True)
        square = Square(side_length=0.6, stroke_color=BLACK, fill_opacity=0.0, stroke_width=3)
    ...
    def _draw_arrow(self, n1, n2):
        # get bottom center of n1 and n2
        # n1 = n1.get_bottom()
        # n2 = n2.get_bottom()
        arrow = Arrow(
            start=n1.get_bottom(),
    ...
    def _draw_graph_node(self, p, label, dim, value):
        ...
        self.play(FadeIn(graph_node), FadeIn(label), FadeIn(val_label), run_time=0.1)
        return graph_node
    ...
    def _draw_line(self, p1, p2, label, label_shift):
        ...
        self.play(ShowCreation(line), FadeIn(label), run_time=0.1)
    ...
    def _title_screen(self, text):
        ...
        title.move_to(ORIGIN)  # Center it
